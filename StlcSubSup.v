...
(* Lemma int_not_sub : forall T,
T = typ_btm \/ T = typ_int \/ ~(R Sub T typ_int).
Proof. destruct T.
- left. reflexivity.
- right. right. intros contra. inversion contra.
- right. left. reflexivity.
- right. right. intros contra. inversion* contra.  Qed.

Corollary sub_int_ident: forall T, R Sub T typ_int -> T = typ_btm \/ T = typ_int.
Proof. intros T H. destruct int_not_sub with (T:=T).
- left. apply H0.
- destruct H0; subst.
  + right. reflexivity.  
  + contradiction. Qed.

Lemma int_sub_ident : forall T, R Sup T typ_int ->
  T = typ_int \/ T = typ_top.
Proof. intros H T. inversion T.
left. reflexivity. 
right. reflexivity. Qed.
  
    
Lemma abs_not_int : forall V T t,
  typing empty (trm_abs V t) T -> ~ (R Sub T typ_int).
Proof. intros V T t H. remember empty as E.
  remember (trm_abs V t) as trm. induction H; subst;try inversion Heqtrm.
  - intros ctr.  apply sub_int_ident in ctr. destruct ctr; inversion H1.
  - intros ctr. apply sub_int_ident in ctr.
    assert (~ R Sub S typ_int). { auto. }
    destruct ctr. subst. inversion H0. subst. apply H1. apply STB2.
    subst. auto. Qed.
 *)
