...
Lemma trans : forall A B m, R m A B -> forall C, R m B C -> R m A C.
  intros A B m s.
  induction s; intros; simpl; destruct m; try inversion H; auto. 
  remember (typ_all T2) as TA. lets M:H1. rewrite HeqTA in M.
   induction H1; subst; inversion* M; subst*; try inversion* HeqTA. 
    destruct m; inversion H3.
    destruct m; inversion H3.
    destruct m; inversion H2.
    destruct m; inversion H5.
    subst. clear HeqTA.
    clear H2. apply SAll with (L := L \u L1). introv NI.
    apply notin_union_r in NI. destruct NI. apply* H0.
    remember (typ_all T2) as TA. lets M:H1. rewrite HeqTA in M.
   induction H1; subst; inversion* M; subst*; try inversion* HeqTA. 
    destruct m; inversion H3.
    destruct m; inversion H3.
