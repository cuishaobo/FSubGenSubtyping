...
| SAll : forall E L m1 m2 T1 T2 T3, wft E T1 -> (forall X, X \notin L ->
...
Proof. introv Rel. 
 inductions Rel; autos*.
- destruct* m.
- destruct* m. 
- split. apply wft_all with (L:=L); autos*. intros. apply H1 in H2. destruct* H2.  
apply wft_all with (L:=L); autos*. intros. apply H1 in H2. destruct* H2.
Qed.      
...
Proof. introv Rel. split; try solve [apply sub_wft in Rel; auto].
induction* Rel. pick_fresh X. assert (X \notin L). auto.
apply H1 in H2. apply okt_push_tvr_inv in H2. destruct* H2. Qed.
...
(*   apply SVarTrns with (X:=X); auto.  *)
  (* case: all *)
  apply SAll with (L:=L \u (dom E) \u (dom F) \u (dom G)). auto. intros. assert (E & F & G & X !: m1 <>: T1 = E&F&(G & X !: m1<>:T1)).
  rewrite concat_assoc. reflexivity. rewrite H3. apply H1; auto.
...
(*   apply SVarTrns with (X:=X); autos*. *)
  apply SAll with (L:=L). apply* wft_narrow. apply ok_from_okt. 
  apply* okt_narrow. pick_fresh X. assert (X \notin L). auto. apply H0 in H2. 
  apply sub_regular in H2. destruct H2. apply okt_push_tvr_inv in H2. destruct* H2. 
  intros. apply H1 with (E0:=E) (Z0:=Z) (m3:=m1) (Q0:=Q) (F0:= (F & X !: m0 <>: T1)) in H2; auto.
  rewrite concat_assoc in H2. auto. rewrite concat_assoc. auto.
Qed.
...
- inductions R2; autos.
  + apply STB1. auto. apply wft_all with (L:=L). autos.
    intros. apply H0 in H4. autos.
  + destruct m; inversion x.
  + assert (forall T0, R E m T T0 -> R E m (typ_fvar X) T0).
     { intros. eapply SVarBnd. apply H3. auto. auto.  }
    clear H3. clear IHR2. inductions R2; autos.
    * destruct m; inversion x.
    * apply sym1. apply H5. apply STB2. auto. 
    apply wft_all with (L:=L). autos.
    intros. apply H3 in H6. autos.
    * apply IHR2 with (m2:=m1) (T4:=T1) (T5:=T3); autos.
      introv Hy. apply H5. eapply SVarBnd. apply H. auto.
      auto.
    * apply sym1. apply H6. apply SAll with (L:=L \u L0); autos.
      introv NI. apply sym2. apply H5; autos. apply sym1.
      apply H0. autos.
  + apply SAll with (L:= L \u L0); autos.
Qed. 

Corollary strns : forall Q, transitivity_on Q.
Proof. intros Q. unfold transitivity_on. introv R1 R2.
eapply sub_transitivity. apply R1. apply R2. Qed. 
...
Proof.
  intros. 
  apply* sub_narrowing_aux. unfold transitivity_on. 
  intros. eapply sub_transitivity. apply H1. auto. 
Qed.
...
Proof.
  ...
  apply H0 in H4. apply sub_regular in H4. destruct H4. apply okt_push_tvr_inv in H4.
  destruct* H4. lets M:PsubQ. apply sub_wft in PsubQ. destruct* PsubQ. 
   unsimpl (subst_tb Z P (bind_tRel m0 T1)).
   intros. rewrite* subst_tt_open_tt_var. rewrite* subst_tt_open_tt_var. 
...
Proof.
  ...
  (*apply SVarTrns with (X:=X); autos*. *)
  (* case: all *)
  apply SAll with (L:= L); autos*. intros. apply_ih_bind* H1.
Qed.
...
Lemma fsub_admissible : forall L E S2 T1 T2,
      (forall X, X \notin L ->
          R (E & X !: Sub <>: T1) Sub (S2 open_tt_var X) (T2 open_tt_var X)) ->
      R E Sub (typ_all Sub T1 S2) (typ_all Sub T1 T2).
Proof. intros. apply SAll with (L:=L).
- simpl. pick_fresh X. assert (X \notin L). { auto. }
  apply H in H0. apply sub_regular in H0. destruct H0. 
  inversion H0. apply empty_concat_inv in H3.
  destruct H3. apply empty_single_inv in H3. false.
  apply eq_push_inv in H2. destruct H2. destruct H6. inversion H6.
  subst. auto. apply eq_push_inv in H2. destruct H2. destruct H6.
  inversion H6.
- simpl. auto. Qed.  
