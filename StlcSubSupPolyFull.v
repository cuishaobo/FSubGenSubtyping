...
Definition SelectBound m2 (S1:typ) (S2:typ) := 
  match m2 with
    | Sup => S1
    | Sub => S2 end.
...
| SVarBnd : forall E X m T F, binds X (bind_tRel m T) E -> wft E T -> R E m T F -> R E m (typ_fvar X) F
| SVarBndFl :  forall E X m T F, binds X (bind_tRel m T) E -> wft E T -> R E m T F -> R E (flip m) F (typ_fvar X)
...
(forall X, X \notin L ->
  R (E&(X !: m1 <>: (SelectBound m2 T1 T2))) m2 (T3 open_tt_var X) (T4 open_tt_var X))-> 
...
induction Rel; autos*. 
  ...
  - split. destruct IHRel; auto. split; apply_fresh* wft_all as Y.
    destruct (H0 Y); auto. destruct H2. destruct m2; simpl in H2.
    apply_empty* (@wft_narrow m1 T2); autos. autos. 
    destruct (H0 Y); auto. destruct H2. destruct m2; simpl in H3.
    autos. apply_empty* (@wft_narrow m1 T1); autos. Qed.
...
Lemma refl : forall E m A,
  ...
  - subst. pick_fresh X. apply SAll with (L:=L\u L0 \u dom E). 
    ...
    apply H0 with (E:=E & v!:m<>:(SelectBound m0 T1 T1)); auto.
    destruct m0; auto.
    destruct m0; auto. 
Defined.
...
(*   - apply SVarTrns with (X:=X); auto. *)
  - apply* SAll. destruct m1; simpl. simpl in IHR. auto.
    simpl in IHR. auto. intros. apply H1 in H2. destruct m2; simpl; 
    simpl in H2; auto.
Defined.
...
(*   - subst. eapply SVarTrns. apply H. apply* IHR2.
  - subst. eapply SVarTrns. apply H. apply* IHR2.  *)- destruct m2; inversion H3.  Qed.
...
Lemma subvar_inv_binds_fln : forall m E X A B,
R E (flip m) (typ_fvar X) A -> binds X (bind_tRel m B) E ->
  A = mode_to_sub (flip m) \/
  (exists X', A = typ_fvar X' /\ ((X' = X) \/ (exists T0, binds X' (bind_tRel m T0) E /\ R E (flip m) (typ_fvar X) T0 ))).
Proof. introv R Bnd. apply subvar_inv in R. destruct R.
- left*.
- destruct H. destruct H. destruct H. apply binds_get in H. apply binds_get in Bnd. 
  rewrite Bnd in H. inversion H. destruct m; inversion H2.
  right. assert (flip (flip m)= m). { destruct m; auto. }
  rewrite H0 in H. auto. Qed.
...
(*   apply SVarTrns with (X:=X); autos*. *)
  ...
  intros. 
  apply H0 with (E0:=E) (Z0:=Z) (m3:=m1) (Q0:=Q) (F0:= (F & X !: m0 <>: SelectBound m2 T1 T2)) in H1; auto.
...
Proof.
  ...
  induction W; intros Q' EQ E S SsubQ;
    induction SsubQ; try discriminate; inversions EQ;
      intros Ty QsubTy; inversions keep QsubTy; 
        eauto 4.
    - destruct m; inversion H2. eapply SVarBnd.
    apply H. auto. apply TopBtmMustEq1; auto.
    - destruct m0; simpls. 
      + eapply SVarBnd. apply H. auto.
        apply TopBtmMustEq1; auto.
      + apply sym2. eapply SVarBnd. simpl. apply H2. auto.
        apply TopBtmMustEq1; auto. eapply wft_var. apply H.
    - eapply SVarBnd. apply H. auto. auto.
    - destruct m; inversion H2.
    - eapply SVarBnd. apply H. auto. apply IHSsubQ; auto.
    - destruct m; inversion H2. eapply SVarBnd.
    apply H. auto. apply TopBtmMustEq1; auto.
   - destruct m0; simpls. 
      + apply sym2. eapply SVarBnd. simpl. apply H2. auto.
    ...
    - destruct m; inversion H2.
    - eapply SVarBnd. apply H. auto. auto.
    - eapply SVarBnd. apply H. auto. apply IHSsubQ.
      auto. eapply SVarBnd. apply H3. auto. auto.
    - eapply SVarBnd. apply H. auto. apply IHSsubQ. auto. auto.
    - destruct m; inversion H1.
    - eapply SVarBndFl. apply H. auto. auto.
    - apply binds_get in H. apply binds_get in H2. rewrite H2 in H. inversion H.
      destruct m; inversion H4.
    - apply sym1 in H5. assert (m = m0). { destruct m; destruct m0; auto. }
      subst. eapply SVarBndFl. apply H2. auto. eapply Var_Trans.
      apply sym2. apply H5. eapply SVarBnd. apply H. auto. auto.
    - destruct m; inversion H.
    - apply SFun. apply sym1. apply IHW1 with (Q':=T1).
      auto. apply sym2. auto. apply sym2. auto.
      apply IHW2 with (Q':=T2). auto. auto. auto.
    - assert (forall T', R E m0 T T' -> R E m0 (typ_fvar X) T').
      { intros. eapply SVarBnd. apply H. auto. auto. }
       clear H. clear IHSsubQ2. clear IHSsubQ1. clear QsubTy.
       inductions H1.
      + destruct m; inversion x.
      + subst. apply sym1. apply TopBtmMustEq1. auto. auto. eapply H2.
       auto. 
      + subst. apply sym1. apply H2. apply SFun.
        apply IHW1 with (Q':=T1). auto. auto.  apply sym2. auto.
        apply sym2. apply IHW2 with (Q':=T2). auto. auto.
        apply sym1. auto.
      + eapply IHR. auto. apply W2. auto. auto. apply W1.
        auto. auto. auto. intros. apply H3. eapply SVarBnd.  apply H. auto.
        auto.
    - destruct m; inversion H2.
    - assert (forall T', R E m T T' -> R E m (typ_fvar X) T').
      { intros. eapply SVarBnd. apply H. auto. auto. }
       clear H. clear IHSsubQ. clear H1. inductions SsubQ.
      + destruct m; inversion x.
      + apply H2. auto.
      + apply H2. apply SFun. apply sym1.
       apply IHW1 with (Q':=T1). auto. apply sym2. auto. apply sym2. auto.
       apply IHW2 with (Q':=T2). auto. auto. auto.
      + eapply IHSsubQ. auto. apply W2. auto. apply W1. auto.
        auto. auto. auto. auto. intros.  apply H2. eapply SVarBnd.
        apply H. auto. auto.
    - eapply SVarBnd. apply H. auto. eapply SVarBndFl.
      apply H2. auto. apply sym2. apply IHSsubQ; auto. apply sym1.
      auto.
    - destruct m; inversion H4.
    - eapply SVarBnd. apply H1. auto. eapply SVarBndFl. apply H4.
      auto. apply sym2. apply IHSsubQ; auto. apply sym1. auto.
    - assert (forall T', R E m T T' -> R E m (typ_fvar X) T').
      { intros. eapply SVarBnd. apply H1. auto. auto. } clear H1. 
      clear H3. inductions SsubQ.
      + destruct m; inversion x.
      + apply H4. auto.
      + apply H4. apply IHSsubQ; auto.
      + apply H4. apply IHSsubQ; auto.
    - apply STB1. auto. apply wft_all with (L:=L0). auto.
      intros. destruct m2; simpls.
      + apply_empty wft_narrow. assert (wft (E & X !: m0 <>: T1) (T4 open_tt_var X)).
        { apply H1 in H5. autos. } apply H6.
       apply ok_from_okt. replace (E & X !: m0 <>: T0) with (E & X !: m0 <>: T0 & empty).
        apply okt_narrow with (V:=T1). rewrite concat_empty_r. apply H1 in H5. auto.
        auto. rewrite concat_empty_r. auto.
      + apply H1 in H5. autos.
    - destruct m2; inversion H3.
    - assert (forall T', R E m T T' -> R E m (typ_fvar X) T').
      { intros. eapply SVarBnd. apply H3. auto. auto. } clear H3. 
      clear IHSsubQ. clear H2. inductions H5.
      + destruct m; inversion x.
      + apply sym1. apply H6. apply STB2. auto. apply wft_all with (L:= L \u L0).
        auto. intros. assert (R (E & X0 !: m0 <>: SelectBound (flip m) T0 T1) 
       (flip m) (T4 open_tt_var X0) (T2 open_tt_var X0)).
      { apply H1. auto. } destruct m; simpls.
        * auto. 
        * replace (E & X0 !: m0 <>: T0) with (E & X0 !: m0 <>: T0 & empty).
          apply wft_narrow with (V:=T1). rewrite concat_empty_r.
          autos. apply ok_from_okt. apply okt_narrow with (V:=T1).
          rewrite concat_empty_r. auto. auto. rewrite concat_empty_r. auto. 
      + eapply IHR. auto. apply H. auto. apply W.
        auto. auto. auto. auto. auto. auto. auto. intros. apply H6.
        eapply SVarBnd. apply H3. auto. auto.
      + apply sym1. apply H6. apply SAll with (L:=L \u L0 \u L1).
        * destruct m0; simpls.
          { apply IHW with (Q':=T1). auto. auto. apply sym2. auto. }
          { apply sym2. apply IHW with (Q':=T1). auto. auto. apply sym1. auto. }
        * simpls. clear H2. clear IHR. intros. apply sym2.
          eapply H0 with (X := X0); autos. destruct m2; simpls.
        { apply H1. autos. }
        { apply_empty (@sub_narrowing_aux m0 Sub T1). unfold transitivity_on.
          intros. destruct m. apply IHW with (Q':=T1).
          auto. auto. auto. apply sym2. apply IHW with (Q':=T1).
          auto. apply sym2. auto. apply sym2. auto. apply H1. auto.
          destruct m0; auto.  }
        destruct m2; simpls. 
        { apply_empty (@sub_narrowing_aux m0 Sup T1). unfold transitivity_on.
          intros. destruct m. apply sym2. apply IHW with (Q':=T1).
          auto. apply sym2. auto. apply sym2. auto. apply IHW with (Q':=T1).
          auto. auto. auto. apply sym2. apply H3. auto.
          destruct m0; auto.  }
        { apply sym2. apply H3. auto. } 
    - apply SAll with (L:=L \u L0 \u L1). 
         * destruct m0; simpls.
          { apply sym1. apply IHW with (Q':=T1). auto. apply sym2. auto. apply sym2. auto. }
          { apply IHW with (Q':=T1). auto. auto. auto. }
        * destruct m2; simpls; intros.
          { apply H0 with (X:=X) (Q':=T2 open_tt_var X). auto.
            auto. replace (E & X !: m0 <>: T5) with (E & X !: m0 <>: T5 & empty).
          apply (@sub_narrowing_aux m0 Sub T1). unfold transitivity_on.
          intros. destruct m.  apply IHW with (Q':=T1). auto.
          auto. auto. auto. apply sym2.  apply IHW with (Q':=T1). auto.
          apply sym2. auto. apply sym2. auto. rewrite concat_empty_r.
          auto. destruct m0; auto. simpl in H9. apply sym2. auto.
          simpl in H9. apply sym2. auto. rewrite concat_empty_r. auto.
          apply H10. auto.   }
          { apply H0 with (X:=X) (Q':=T2 open_tt_var X). auto.
            auto. auto.
          replace (E & X !: m0 <>: T0) with (E & X !: m0 <>: T0 & empty).
           apply (@sub_narrowing_aux m0 Sup T1). unfold transitivity_on.
          intros. destruct m. apply sym2.  apply IHW with (Q':=T1). auto.
          apply sym2. auto. apply sym2. auto. apply IHW with (Q':=T1). auto.
          auto. auto. rewrite concat_empty_r.
          auto. destruct m0; auto.  rewrite concat_empty_r. auto.
            } 
Qed.
...
Proof.
  ...
  apply SAll with (L:=L \u \{Z}). apply* IHSsubT.
   (*apply* wft_subst_tb. apply sub_wft in PsubQ. destruct* PsubQ. 
  apply ok_from_okt. apply* okt_subst_tb. pick_fresh X. assert (X \notin L). auto. 
*)  lets M:PsubQ. apply sub_wft in PsubQ. destruct* PsubQ.
  destruct m2; simpl; simpl in H0. 
   unsimpl (subst_tb Z P (bind_tRel m0 T2)).
   intros. rewrite <- concat_assoc. rewrite <- map_push.
  rewrite* subst_tt_open_tt_var. rewrite* subst_tt_open_tt_var.
   apply H0 with (m2:=m1) (Q0:=Q); autos. rewrite concat_assoc. auto.
...
Proof.
 ...
 apply refl with (E:=E). destruct Typ. destruct H1. auto. *)
   ...
   apply (typing_through_subst_te H6). 
    eapply sub_transitivity. apply H. destruct H7.
    destruct H8. apply sym2. auto.
...
Proof.
  ...
  right. destruct* IHTyp as [Val | [e1 Red]].
      destruct (canonical_form_tabs Val Typ).
        subst. destruct H0. destruct H0. subst. exists* (open_te x0 T2). 
  autos*.
Qed.

Lemma kernel_admissible : forall E L m1 m2 T1 T2 T3, wft E T1 -> (forall X, X \notin L ->
  R (E&(X !: m1 <>: T1)) m2 (T2 open_tt_var X) (T3 open_tt_var X))-> 
  R E m2 (typ_all m1 T1 T2) (typ_all m1 T1 T3).
Proof. intros. apply SAll with (L:=L).
  - apply* refl. pick_fresh X. assert (X \notin L). {auto. }
    apply H0 in H1. apply okt_strengthen_l with (F:=X !: m1 <>: T1).
    auto.
  - destruct m2; auto.
Qed.

Lemma fsub_admissible : forall L E S1 S2 T1 T2,
      R E Sub T1 S1 ->
      (forall X, X \notin L ->
          R (E & X !: Sub <>: T1) Sub (S2 open_tt_var X) (T2 open_tt_var X)) ->
      R E Sub (typ_all Sub S1 S2) (typ_all Sub T1 T2).
Proof. intros. apply SAll with (L:=L).
- simpl. apply sym2. auto.
- simpl. auto. Qed.  
